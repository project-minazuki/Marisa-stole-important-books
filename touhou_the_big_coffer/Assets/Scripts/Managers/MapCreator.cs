...
public class MapCreator : MonoBehaviour
{
    ...
    private void Create()
    {
        if (newWallx - positionx <= 20)
        {
            ...
            if (Random.Range(0, 100) >= 90 - positionx / 200 && usedY[0] != -3) Instantiate(stab, new Vector3(newWallx, -3.35f, 0), gameObject.transform.rotation);
            if (Random.Range(0, 100) >= 80 - positionx / 200)
            {
                if (Random.Range(0, 100) >= 20f | positionx <= 50) Instantiate(stab, new Vector3(newWallx, usedY[0] + 0.65f, 0), gameObject.transform.rotation);
                else
                {
